...
client = None
last_order_time = 0
ORDER_COOLDOWN = 3  # กันยิงถี่เกิน (วินาที)
...
def safe_create_order(**kwargs):
    ...
    if now - last_order_time < ORDER_COOLDOWN:
        ...
        return {"status": "cooldown blocked"}
    ...
    try:
        ...
        if e.code == -1000:
            print("⚠ Rate limit hit. Retrying in 2 sec...")
            time.sleep(2)
            order = client.futures_create_order(**kwargs)
            last_order_time = time.time()
            return order
...
def webhook():
    ...
    if action == "OPEN":
        ...
        try:
            # ⚠ เปลี่ยน leverage เฉพาะตอนจำเป็น
            ...
            return jsonify({"status": "opened", "result": result})
